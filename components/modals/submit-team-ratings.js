  ...
  run: async (client, interaction) => {
        ...
        return interaction.reply({
          ...
          components: [
            new ActionRowBuilder().addComponents(
              new ButtonBuilder()
                .setCustomId(`match-gamemode_${analysisTimestamp}_${gamemode}`)
                .setLabel('Try Again')
                .setStyle('Primary')
                .setEmoji('🔄')
            ),
    ...
    for await (const embarkId of pointData.keys()) {
      ...
      if (userDocs.length == 0) {
        log(`User with embark ID ${embarkId} not found in the database`, 'warn')
        //continue;
              ...
              new ButtonBuilder()
                .setCustomId(
                  `set-match-embarkid_${gamemode}_${analysisTimestamp}_${embarkId
                    .replaceAll('_', '~')
                    .replaceAll(' ', '')}_${interaction.fields.fields
                    .map((field) => {
                      if (confirmWords.includes(field.value)) return 'y'
                      if (denyWords.includes(field.value)) return 'n'
                    })
                    .join(',')}`
                )
    ...
    for await (const embarkId of playerDocs.keys()) {
      ...
      if (!playerPointData) {
        log(`Point data for user with embark ID ${embarkId} not found`, 'warn')
        log(`Seaching for ${embarkId.toLowerCase()}`, 'debug')
