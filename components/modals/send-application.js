...
export default {
  ...
  run: async (client, interaction) => {
    ...
    const seasonsPlayed = interaction.fields.getTextInputValue('seasons-played')
    ...
    seasons = seasons.map((season) => {
      ...
      if (client.config.seasons.map((s) => s.name).includes(season)) {
        return client.config.seasons.find((s) => s.name == season).value
      }
    ...
    if (lastRecordedRank.split(' ').length > 2) {
      lastRecordedRank = lastRecordedRank.split(' ').slice(0, 2).join(' ')
    }
    if (
    ...
    if (highestRecordedRank.split(' ')[0] == 'plat') {
      highestRecordedRank = highestRecordedRank.replace('plat', 'platinum')
    }
    if (
    ...
    /* Validate wins, losses, eliminations, deaths, and matches played */
    // if (!wins.match(/^[0-9]+$/)) {
    //   await interaction.reply({
