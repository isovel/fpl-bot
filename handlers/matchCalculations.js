...
export default {
  ...
  submitPointData: async (
    ...
    if (
      userDoc.matches?.find((match) => match.timestamp == analysisTimestamp)
    ) {
    ...
    let userPointData = {
      ...
      kills: pointData.pointSources.find((source) => source.source == 'Kills'),
      kda: pointData.pointSources.find((source) => source.source == 'kda'),
    ...
    const embedData = [
      ...
      seperator,
      `Score Achieved: ${Math.round((pointData.totalPoints / 150) * 1000)}`,
      ...
      `Team Results: ${toNumStr(userPointData.placement.index)} - ${Math.round(
      ...
      `Objective Played: ${userPointData.objective.index ? 'Yes' : 'No'} - ${
        userPointData.objective.points
      } points`,
    ]
