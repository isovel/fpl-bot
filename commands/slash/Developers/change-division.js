...
export default {
  ...
  run: async (client, interaction) => {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription('The user was not found in the database.')
            .setColor('Red'),
        ],
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription('The user is already in the specified division.')
            .setColor('Red'),
        ],
        ephemeral: client.config.development.ephemeral,
      })
    ...
    try {
      ...
      await member.roles.remove(client.config.roles.divisions[userDoc.division])
      await member.roles.add(client.config.roles.divisions[division])
      await c_users.updateOne({ discordId: user.id }, { $set: { division } })
      if (sendDm) {
        await user.send({
          embeds: [
            new EmbedBuilder()
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle('Error')
            .setDescription('An error occurred while changing the division.')
            .setColor('Red'),
        ],
        ephemeral: client.config.development.ephemeral,
      })
